...
public final class EmulatorUtil {
	private static final Logger LOG = Logger.getLogger(EmulatorUtil.class
			.getSimpleName());
	 ...
	 *            the app. engine Application URL as
	 *            https://<yourapp>.appspot.com
	 * 
	 * @return the service defintion for the Mirror.
	 */
	public static Mirror getMirror(Credential credential,
			String applicationName, String appEngineApplicationURL) {

		return getMirror(credential, applicationName, appEngineApplicationURL,
	...
	private static final void setAppEngineApplicationURL(String appURL) {
		...
		if (StringUtils.isNotBlank(appURL)) {
			...
			throw new RuntimeException(
					"Unable to set your app engine application URL.");
		}
	}
	...
	public static final String getAppEngineApplicationURL() {
		...
		if (StringUtils.isNotBlank(appURL)) {
			...
			throw new RuntimeException(
					"Unable to get your app engine application URL.  "
	 ...
	 * Emulator not, we will transformt the card in one HTML TimeLineItem.
	 * https://github.com/Scarygami/mirror-api - REAMDME.md
	 * 
	 * @param content
	 *            The initial timeline item.
	 * @param mediaContent
	 *            The attachement image.
	 * @return The initial timeline item modified to look like one HTML card
	 *         with the atachement.
	 * @throws IOException
	 * @throws FileNotFoundException
	 */

	public static TimelineItem createTimeLineItemWithAtachement(
			TimelineItem content, AbstractInputStreamContent mediaContent)
	 ...
	 * 
	 * @param mediaContent
	 *            the attachement to transform into URL
	 * @return The attachement return URL
	 * @throws IOException
	 * @throws FileNotFoundException
	 */
	public static String storeAttachement(
	...
	public static void setGlasswareHost(String glasswareHost) {
		...
		EmulatorUtil.glasswareHost = glasswareHost.endsWith("/") ? glasswareHost
				.substring(0, glasswareHost.length() - 1) : glasswareHost;
	}
